...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("mousemove", (e) => {
    mouseX = e.clientX;
    mouseY = e.clientY;
    cursor.style.opacity = 1; // fade in when moving
  });

  document.addEventListener("mouseleave", () => {
    cursor.style.opacity = 0; // fade out when leaving window
  });

  // Grow on interactive elements
  const interactiveElements = document.querySelectorAll("a, button, .feature-box, .destination-card, .testimonial-card");
